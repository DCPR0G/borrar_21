    ...
    for (let i = 0; i < num; i++) {
      ...
      if(parseInt(spendCounter.innerHTML) > 11000){
        ...
        if(parseInt(spendCounter.innerHTML) > 5000){
          voy_reward = voyGachaItems[Math.floor(Math.random()*234 + 5)]
          console.log("gasté más de 4k pero menos de 11k")
        }
        else{
          voy_reward = voyGachaItems[Math.floor(Math.random()*218+ 21)]
...
const WarningMessage = document.querySelector(".development_message")

const WarningMessageButton = document.querySelector(".close_warning")

WarningMessageButton.addEventListener("click",()=>{
